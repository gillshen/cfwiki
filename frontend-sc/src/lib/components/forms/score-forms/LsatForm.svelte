<script lang="ts">
	...
	import type { LsatScoreSchema } from '$lib/schemas/scores';
	import Input from '$lib/components/forms/Input.svelte';

	export let data: SuperValidated<Infer<LsatScoreSchema>>;
	export let onUpdated: (event: { form: typeof data }) => any;
	export let action: string = '?/createOrUpdateLsatScore';
	export let className: string = '';
	export let studentId: number;
	export let score: LsatScore | undefined = undefined;
