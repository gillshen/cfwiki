<script lang="ts">
	...
	import type { GreScoreSchema } from '$lib/schemas/scores';
	import Input from '$lib/components/forms/Input.svelte';

	export let data: SuperValidated<Infer<GreScoreSchema>>;
	export let onUpdated: (event: { form: typeof data }) => any;
	export let action: string = '?/createOrUpdateGreScore';
	export let className: string = '';
	export let studentId: number;
	export let score: GreScore | undefined = undefined;
